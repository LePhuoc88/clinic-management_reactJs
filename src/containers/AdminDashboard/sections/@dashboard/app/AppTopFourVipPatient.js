...
export default function AppTopFourVipPatient() {
    ...
    useEffect(() => {
        ...
        if (fourVipPatient.length !== 0) {
            fourVipPatient.map((item) => {
                const lastName = item.patientDataInvoice?.lastName || ''; // Giá trị mặc định là chuỗi rỗng
                const firstName = item.patientDataInvoice?.firstName || ''; // Giá trị mặc định là chuỗi rỗng
                let name = `${lastName} ${firstName}`.trim(); // Xóa khoảng trắng thừa
                CHART_DATA.push(parseInt(item.total_revenue));
                LABELS_DATA.push(name || 'Bệnh nhân không rõ'); // Giá trị mặc định "Bệnh nhân không rõ" nếu tên rỗng
            });

            if (CHART_DATA.length > 0) {
                setChartData(CHART_DATA);
            }
            if (LABELS_DATA.length > 0) {
                chartOptions.labels = LABELS_DATA;
                setChartOptionsState({ ...chartOptions });
            }
        }
    ...
    return (
        <Card>
            ...
            <ChartWrapperStyle dir="ltr">
                <ReactApexChart type="pie" series={chartData} options={chartOptionsState} height={280} />
            </ChartWrapperStyle>
        </Card>
    );
}
